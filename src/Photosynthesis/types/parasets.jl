...
KcTDBernacchi(FT) =
    ArrheniusTD{FT}(41.0264925, 79430.0 / GAS_R(), 79430.0 / RT₂₅(FT));

""" [`ArrheniusTD`](@ref) type Ko TD from Bernacchi's data """
KoTDBernacchi(FT) =
    ArrheniusTD{FT}(  28208.88, 36380.0 / GAS_R(), 36380.0 / RT₂₅(FT));

""" [`ArrheniusTD`](@ref) type Respiration TD from Bernacchi's data """
RespirationTDBernacchi(FT) =
    ArrheniusTD{FT}(       Inf, 46390.0 / GAS_R(), 46390.0 / RT₂₅(FT));

""" [`ArrheniusTD`](@ref) type Vcmax TD from Bernacchi's data """
VcmaxTDBernacchi(FT) =
    ArrheniusTD{FT}(       Inf, 65330.0 / GAS_R(), 65330.0 / RT₂₅(FT));

""" [`ArrheniusTD`](@ref) type Vomax TD from Bernacchi's data """
VomaxTDBernacchi(FT) =
    ArrheniusTD{FT}(       Inf, 60110.0 / GAS_R(), 60110.0 / RT₂₅(FT));

""" [`ArrheniusTD`](@ref) type ``Γ^{*}`` TD from Bernacchi's data """
ΓStarTDBernacchi(FT) =
    ArrheniusTD{FT}(4.33164375, 37830.0 / GAS_R(), 37830.0 / RT₂₅(FT));
...
KcTDCLM(FT) =
    ArrheniusTD{FT}(  40.49, 79430.0 / GAS_R(), 79430.0 / RT₂₅(FT));
